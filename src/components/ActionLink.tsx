...
export default function ActionLink({ href, children, className = '', ariaLabel }: ActionLinkProps) {
  return (
    <a
      href={href}
      className={`inline-flex items-center gap-2 transition-all duration-500 group hover:text-secondary ${className}`}
      aria-label={ariaLabel ?? (typeof children === 'string' ? children : undefined)}
    >
      <span>{children}</span>
      <div className="bg-primary p-2 rounded group-hover:bg-secondary">
